...
import { AppTag } from '@/components/ui/AppTags'
...
import type { ContentCorrectionSession } from '@/types/admin'

function AdminContentCorrectionPage() {
  ...
  const accent = role ? AppColors.role[role] : AppColors.role.admin
  const quickActions = [
    'Pedir mais detalhes',
    ...
    async function loadCorrectionSession() {
      ...
      try {
        const currentSession =
          await adminApprovalService.getContentCorrectionSession(contentId)
        const activeSession =
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      const nextSession =
        await adminApprovalService.sendContentCorrectionMessage(contentId, {
          body: draftMessage,
        })
...
export default AdminContentCorrectionPage
